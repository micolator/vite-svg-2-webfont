        ...
        async buildStart() {
            ...
            if (isBuild) {
                const emitted = processedOptions.types.map<[T, string]>(type => [
                    type,
                    `${processedOptions.buildFontsBaseUrl}/${this.getFileName(this.emitFile({ type: 'asset', fileName: `assets/${processedOptions.fontName}-${guid()}.${type}`, source: generatedFonts?.[type] }))}`,
                ]);
